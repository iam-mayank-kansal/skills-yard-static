...
const courseCon = document.querySelector(".computer-courses-container");
const courseBtn = document.querySelectorAll(".computer-Courses-btns");
const moveAmount = 875;
let click = 0;
const coursesDot = document.querySelectorAll(".computer-Courses-dots-para span");

courseBtn.forEach((a) => {
    a.addEventListener("click", () => {
        courseCon.scrollLeft += a.id === "computer-Courses-right-btn" ? moveAmount : -moveAmount;
    })
})

courseBtn[0].addEventListener("click", () => {
    if (click == 1) {
        for (let x of coursesDot) {
            x.style.color = "black";
        }
        coursesDot[0].style.color = "#0c6c";
        click--;
    }
    else if (click == 2) {
        for (let x of coursesDot) {
